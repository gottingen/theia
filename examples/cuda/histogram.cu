...
int main(void) {
    ...
    THEIA_CUDA_CHECK(cudaMalloc((void**)&state, NBINS * sizeof(curandState_t)));
    // clang-format off
    ...
    THEIA_CUDA_CHECK(cudaMalloc((void**)&histOut, NBINS * sizeof(int)));
    THEIA_CUDA_CHECK(
    ...
    THEIA_CUDA_CHECK(cudaFree(histOut));
    THEIA_CUDA_CHECK(cudaFree(histColors));
...
void destroyBitmap(Bitmap& bmp) { THEIA_CUDA_CHECK(cudaFree(bmp.ptr)); }

PerlinNoise::PerlinNoise() {
    const size_t IMG_SIZE = IMGW * IMGH * sizeof(float);

    THEIA_CUDA_CHECK(cudaMalloc((void**)&base, IMG_SIZE));
    THEIA_CUDA_CHECK(cudaMalloc((void**)&perlin, IMG_SIZE));
}

PerlinNoise::~PerlinNoise() {
    THEIA_CUDA_CHECK(cudaFree(base));
    THEIA_CUDA_CHECK(cudaFree(perlin));
}
